...
--
-- Everything from 064 to 103, in order, as one file.
--
-- Your production database is on 063. This brings it to 103.
--
-- HOW TO RUN
...
--   087      puts the 16 staff into app_user, so the segregation-of-duties
--            role check can fire at all — it had nobody to be wrong about.
--   088      makes the journal the SCREEN posts honour the approval threshold.
--            It did not, so a threshold set in System admin had no effect on
--            anything posted from the interface.
--   089      a trust distribution now checks the fund it is paid from. It did
--            not, so income could be distributed from a fund holding nothing.
--   090      three more functions that skipped their own controls now delegate
--            to the guarded versions, including approving statutory accounts.
--   091      TAKES EXECUTE AWAY FROM THE ANONYMOUS KEY. It could call 270
--            functions, 101 of which write and run as the owner.
--   092      a row-level audit trail on the records that get disputed.
--   093      releasing a payment run now refuses whoever approved it. The
--            separation was two-way and documented as three-way.
--   094      a real entity can no longer be turned into demo data.
--   095      a missing FX rate refuses instead of being treated as zero, which
--            produced a plausible and badly wrong translation adjustment.
--   096      two more: billable time with no rate was worth nothing, and a
--            reducing-balance asset with no rate never depreciated.
--   097      a missing currency inherits the entity's own rather than
--            defaulting to sterling. Twelve of 22 entities are not GBP.
--   098      a set of accounts can no longer be finalised without being
--            approved. A draft went straight to filed, skipping nine gates.
--   099      closes direct table reads on anything that will hold client data.
--            RBAC gates the interface; it did not gate the API.
--   100      charge-out rates and services, so time can be recorded at all.
--            There was no rate table of any kind.
--   101      FATCA and CRS classification as reference data rather than free
--            text, with what each classification means for reporting.
--   102      the questions that arrive at a classification, the record of the
--            answers, and the reporting extract that feeds a return.
--   103      a note on a CPD entry, categories as structured or general, and a
--            person's own log with the structured/general split.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ───────────────────────────────────────────────────────────────────────
-- 088_journal_post_honours_threshold.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 089_distribution_checks_the_fund.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 090_close_the_remaining_bypasses.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 092_audit_the_records_that_get_disputed.sql
-- ───────────────────────────────────────────────────────────────────────
-- 092 — a row-level audit trail on the records that get disputed
--
...
-- ───────────────────────────────────────────────────────────────────────
-- 093_the_third_leg_of_the_payment_control.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 095_missing_fx_rate_is_not_zero.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 096_a_missing_rate_is_not_zero_either.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 097_a_missing_currency_is_not_sterling.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 098_finalising_requires_approval.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 100_charge_rates_and_services.sql
-- ───────────────────────────────────────────────────────────────────────
-- 100 — charge-out rates and services, so time can be recorded at all
--
...
-- ───────────────────────────────────────────────────────────────────────
-- 101_fatca_crs_classification.sql
-- ───────────────────────────────────────────────────────────────────────
...
-- ───────────────────────────────────────────────────────────────────────
-- 102_fatca_methodology_and_extract.sql
-- ───────────────────────────────────────────────────────────────────────
